...
export default function ExamScheduleCard() {
  const nextExam = getNextExamDate(SQLD_SCHEDULES);
  const dday = nextExam ? getDday(nextExam.examDate) : null;
      ...
      <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 md:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">2026년 SQLD 시험 일정</h2>
              <p className="text-xs text-slate-400">SQL 개발자 (제60회 ~ 제63회)</p>
            </div>
          </div>
          {nextExam && dday !== null && dday >= 0 && (
            <div className="flex items-center gap-2 bg-blue-500/10 border border-blue-500/30 rounded-xl px-4 py-2">
              ...
              <span className="text-xs text-slate-400">
                다음 시험 | 제{nextExam.round}회 {formatExamDate(nextExam.examDate)}
              </span>
            </div>
          )}
        </div>
