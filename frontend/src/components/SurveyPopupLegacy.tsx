...
interface SurveyPopupLegacyProps {
  campaignKey: string;
  onClose: (dismissForToday: boolean, hideUntilCampaignEnd?: boolean) => void;
}

export default function SurveyPopupLegacy({ campaignKey, onClose }: SurveyPopupLegacyProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-8 py-6 max-h-[60vh] overflow-y-auto space-y-6">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-2">Q4. 추가 의견이 있다면 남겨주세요</p>
            <textarea
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              placeholder="서비스를 이용하며 느낀 점이나 개선 의견을 자유롭게 작성해주세요."
              className="w-full min-h-28 px-4 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
            />
          </div>
