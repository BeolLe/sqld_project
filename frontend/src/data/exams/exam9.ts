...
// SQLD 모의고사 9회 (고급 난이도 - 실제 시험의 110% 수준)
...
export const EXAM_9_PROBLEMS: Problem[] = [
  ...
  {
    id: 'exam9_p11',
    title: '11. REGEXP_LIKE와 복합 조건',
    description:
      '다음 SQL에서 유효한 한국 휴대폰 번호 형식(010-XXXX-XXXX)만 조회하는 정규표현식으로 올바른 것은?\n\n```sql\nSELECT PHONE\nFROM CUSTOMERS\nWHERE REGEXP_LIKE(PHONE, _____);\n```',
    type: 'multiple_choice',
    difficulty: 'hard',
    category: '정규표현식',
    correctRate: 30,
    answer: '4',
    explanation:
      '한국 휴대폰 번호 010-XXXX-XXXX 형식을 검증하려면: ^010은 010으로 시작, -는 하이픈, [0-9]{4}는 숫자 정확히 4자리, $는 문자열 끝을 의미합니다. 전체 패턴은 ^010-[0-9]{4}-[0-9]{4}$입니다.',
    options: [
      '\'^010[0-9]{8}$\'',
      '\'^010-[0-9]+-[0-9]+$\'',
      '\'^[0-9]{3}-[0-9]{4}-[0-9]{4}$\'',
  ...
  {
    id: 'exam9_p18',
    title: '18. UNPIVOT에서 NULL 처리',
    description:
      '다음 UNPIVOT 쿼리에 대한 설명으로 올바른 것은?\n\n```sql\n-- 원본 테이블 DATA (2행)\n-- ID  COL_A  COL_B  COL_C\n-- 1   100    NULL   300\n-- 2   NULL   200    NULL\n\nSELECT * FROM DATA\nUNPIVOT INCLUDE NULLS (VAL FOR COL_NAME IN (COL_A, COL_B, COL_C));\n```',
    type: 'multiple_choice',
    difficulty: 'hard',
    ...
    answer: '3',
    explanation:
      'UNPIVOT은 기본적으로 NULL 값을 가진 행을 제외합니다(EXCLUDE NULLS가 기본값). INCLUDE NULLS를 지정하면 NULL 값도 포함하여 변환합니다. 원본 2행 × 3컬럼이므로 INCLUDE NULLS 시 6행, 기본(EXCLUDE NULLS) 시 NULL이 아닌 값만 포함하여 3행이 됩니다.',
    options: [
      '결과는 3행이다.',
      '결과는 4행이다.',
      '결과는 6행이며, NULL 값도 포함된다.',
      'INCLUDE NULLS는 유효하지 않은 구문이다.',
    ],
    points: 10,
  },
  {
