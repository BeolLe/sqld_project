...
// SQLD 모의고사 10회 (고급 난이도 - 실제 시험의 110% 수준)
...
export const EXAM_10_PROBLEMS: Problem[] = [
  ...
  {
    id: 'exam10_p16',
    title: '16. 정규표현식 함수 비교',
    description:
      '다음 Oracle 정규표현식 함수에 대한 설명으로 올바르지 않은 것은?',
    type: 'multiple_choice',
    difficulty: 'hard',
    category: '정규표현식',
    correctRate: 30,
    answer: '4',
    explanation:
      'REGEXP_LIKE는 패턴 일치 여부를 반환(WHERE 절 전용), REGEXP_SUBSTR은 매칭된 부분 문자열 반환, REGEXP_REPLACE는 패턴 치환, REGEXP_COUNT는 매칭 횟수를 반환합니다. REGEXP_INSTR은 매칭 위치(숫자)를 반환하며, 부분 문자열을 반환하지 않습니다.',
    options: [
      'REGEXP_LIKE는 조건절(WHERE)에서 사용하며 TRUE/FALSE를 반환한다.',
      'REGEXP_SUBSTR은 매칭된 부분 문자열을 반환한다.',
      'REGEXP_COUNT는 패턴이 매칭되는 횟수를 반환한다.',
      'REGEXP_INSTR은 매칭된 부분 문자열을 반환한다.',
    ],
    points: 10,
  },
  {
  ...
  {
    id: 'exam10_p23',
    title: '23. PIVOT과 UNPIVOT 종합',
    description:
      '다음 중 PIVOT과 UNPIVOT에 대한 설명으로 올바르지 않은 것은?',
    type: 'multiple_choice',
    difficulty: 'hard',
    ...
    answer: '1',
    explanation:
      'PIVOT은 집계함수가 필수이지만, UNPIVOT은 집계함수 없이 열→행 변환만 수행합니다. PIVOT의 IN 절에는 고정 값 목록 또는 XML 옵션으로 서브쿼리를 사용할 수 있고, UNPIVOT은 기본적으로 NULL을 제외하지만 INCLUDE NULLS로 포함시킬 수 있습니다.',
    options: [
      'PIVOT과 UNPIVOT 모두 집계함수가 필수이다.',
      'PIVOT은 행→열, UNPIVOT은 열→행 변환을 수행한다.',
      'UNPIVOT은 기본적으로 NULL 값을 제외한다.',
      'PIVOT의 FOR 절은 피벗 대상 컬럼을 지정한다.',
    ],
    points: 10,
  },
  // --- DML (24~28) ---
