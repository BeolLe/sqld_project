...
// SQLD 모의고사 8회 (고급 난이도 - 실제 시험의 110% 수준)
...
export const EXAM_8_PROBLEMS: Problem[] = [
  ...
  {
    id: 'exam8_p20',
    title: '20. PIVOT XML과 동적 피벗',
    description:
      'Oracle PIVOT XML에 대한 설명으로 올바른 것은?',
    type: 'multiple_choice',
    difficulty: 'hard',
    ...
    answer: '2',
    explanation:
      'PIVOT XML은 IN 절에 서브쿼리나 ANY를 사용할 수 있어 동적 피벗이 가능합니다. 일반 PIVOT은 IN 절에 고정된 값 목록만 허용하지만, PIVOT XML은 IN (SELECT DISTINCT ... FROM ...) 또는 IN (ANY) 형태를 지원합니다. 결과는 XML 형식으로 반환됩니다.',
    options: [
      'PIVOT XML은 일반 PIVOT과 완전히 동일하게 동작한다.',
      'PIVOT XML은 IN 절에 서브쿼리를 사용하여 동적 피벗이 가능하다.',
      'PIVOT XML의 결과는 JSON 형식으로 반환된다.',
      'PIVOT XML은 Oracle에서 지원하지 않는다.',
    ],
    points: 10,
  },
  {
    id: 'exam8_p21',
    title: '21. REGEXP_REPLACE 고급 활용',
    description:
      '다음 SQL의 실행 결과로 올바른 것은?\n\n```sql\nSELECT REGEXP_REPLACE(\'abcDEFghi\', \'([a-z]+)([A-Z]+)([a-z]+)\', \'\\3-\\2-\\1\') AS RESULT\nFROM DUAL;\n```',
    type: 'multiple_choice',
    difficulty: 'hard',
    category: '정규표현식',
    correctRate: 30,
    answer: '3',
    explanation:
      'REGEXP_REPLACE에서 \\1, \\2, \\3은 캡처 그룹의 역참조입니다. 입력 문자열에서 \\1=abc, \\2=DEF, \\3=ghi로 캡처되며, \\3-\\2-\\1로 재배치하면 ghi-DEF-abc가 됩니다.',
    options: [
      'abc-DEF-ghi',
      'DEF-abc-ghi',
      'ghi-DEF-abc',
